...
export default function App() {
  ...
  const handleSearch = async (query: string) => {
    ...
    try {
      ...
      // 4. Gemini AI 분석 (캐시 우선, rate-limit 시 자동 재시도)
      setStep(3)
      ...
      if (!aiAnalysis) {
        ...
        let aiRes = await callAnalyze()

        // 429: Gemini rate-limited → 서버가 알려준 시간만큼 기다렸다 재시도 (최대 3회)
        for (let retry = 0; aiRes.status === 429 && retry < 3; retry++) {
          const { retryAfterMs } = await aiRes.json() as { retryAfterMs: number }
        ...
        aiAnalysis = await aiRes.json() as Analysis
